...
type Props = {
  ...
  /** Taille de police en px (préparation zoom, défaut 16) */
  fontSizePx?: number;
};
...
export const EditorPane = forwardRef<EditorPaneRef, Props>(function EditorPane(
  { value, onChange, height, onChordAtCursorChange, fontSizePx },
  ref
) {
  ...
  return (
    <div
      className={`chordpro-editor min-w-0 overflow-hidden rounded-xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white/90 dark:bg-zinc-950/30 backdrop-blur-sm flex flex-col ${fillParent ? "h-full min-h-0" : ""}`}
      style={fontSizePx != null ? { ["--chordpro-editor-font-size" as string]: `${fontSizePx}px` } : undefined}
    >
